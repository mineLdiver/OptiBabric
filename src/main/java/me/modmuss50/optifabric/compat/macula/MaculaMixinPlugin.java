...
public class MaculaMixinPlugin extends InterceptingMixinPlugin {
    @Override
            ...
            case "GameRendererMixin" -> {
                ...
                for (MethodNode method : targetClass.methods) {
                    if (delta.equals(method.name) && deltaDesc.equals(method.desc)) {
                        ...
                        String method_1548Desc = "(Lnet/minecraft/class_127;ID)I"; // int (LivingEntity, int, double)
                        String method_1548 = RemappingUtils.getMethodName("class_471", "method_1548", method_1548Desc); // WorldRenderer, method_1548
                        method_1548Desc = RemappingUtils.mapMethodDescriptor(method_1548Desc);
                        ...
                        extra.add(new InsnNode(ICONST_0));
                        extra.add(new LdcInsnNode(0D));
                        extra.add(new MethodInsnNode(INVOKEVIRTUAL, worldRenderer, method_1548, method_1548Desc));
                        ...
                        for (AnnotationNode annotation : method.visibleAnnotations) {
                            if (mixinMerged.equals(annotation.desc) && annotation.values != null) {
                                for (int i = 0; i < annotation.values.size(); i+=2) {
                                    ...
                                    if (
                                            "mixin".equals(key) && "net.mine_diver.macula.mixin.TessellatorMixin".equals(annotation.values.get(i + 1)) &&
                    ...
                    if (method.visibleAnnotations != null) {
                        ...
                        for (AnnotationNode annotation : method.visibleAnnotations) {
                            if (mixinMerged.equals(annotation.desc) && annotation.values != null) {
                                for (int i = 0; i < annotation.values.size(); i+=2) {
                                    String key = (String) annotation.values.get(i);
                                    if ("mixin".equals(key) && "net.mine_diver.macula.mixin.BlockRenderManagerMixin".equals(annotation.values.get(i + 1))) {
                                        method.instructions.clear();
                                        method.maxStack = 0;
                                        method.maxLocals = 0;
                                        method.instructions.add(new InsnNode(RETURN));
